...
class ServerResponse(NamedTuple):
    ...
    reasoning: str  # reasoning/thinking tokens (empty for non-reasoning models)
    num_chunks: int
...
def _conversation_id_disabled() -> bool:
    return os.environ.get("DISABLE_CONVERSATION_ID", "").lower() in (
        "1",
        "true",
        "yes",
    )
...
async def send_request(
    ...
    if conversation_id is not None and not _conversation_id_disabled():
    ...
    if reasoning_effort:
        payload["chat_template_args"] = {"reasoning_effort": reasoning_effort}
    ...
    async with session.post(
        ...
        if http_status == HTTPStatus.OK:
            async for chunk_bytes in response.content:
                ...
                # Split in case of multiple lines in the buffer
                for line in decoded_chunk.splitlines():
                    ...
                    if stream is False:
                        ...
                        reasoning = delta.get("reasoning", None) or delta.get("reasoning_content", None)
                        ...
                        if content or reasoning:
                            if ttft is None:
                                ...
                                first_chunk = content or reasoning
                            else:
                                # Decoding phase
...
async def send_turn(
    ...
    output_num_tokens = get_token_count(tokenizer, output_content) + get_token_count(tokenizer, response.reasoning)
...
def process_statistics(
    ...
    for percent in warmup_percentages:
        ...
        with pd.option_context(
            ...
            print(df.to_string(float_format=lambda value: f"{value:.2f}"))
        print(TEXT_SEPARATOR)
